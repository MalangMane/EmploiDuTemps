...
class AffichageEmploiDuTemps(Canvas):
            ...
            for cours in listCoursBySemaine[i]:
                if listCoursBySemaine[i].__len__() != 0 : 
                    if cours["heureCours"].__contains__("AM") :
                        self.courscomponents.append(CoursComponent(canvas = self, jour=i, heure=1, coursRow=cours))
                    if cours["heureCours"].__contains__("PM") :
                        self.courscomponents.append(CoursComponent(canvas = self, jour=i, heure=2, coursRow=cours))
    ...
    def refreshCoursComponent(self,*args,entrys):
        listCoursBySemaine = self.controller.selectCours(self.currentdate,entrys["Classe"].get())
    ...
    def nextWeek(self,entrys,label):
        ...
        date = datetime.strptime(derniereDateStocke, '%d-%m-%Y') + timedelta(days=7)
        listCoursBySemaine = self.controller.selectCours(date,entrys["Classe"].get())
    ...
    def selectSemaineByDate(self, entrys,label):
        dateSelect = "{0}-{1}-{2}".format(entrys["Jour"].get(),entrys["Mois"].get(),entrys["Annee"].get())
        dateSelect = datetime.strptime(dateSelect, '%d-%m-%Y')
        listCoursBySemaine = self.controller.selectCours(dateSelect,entrys["Classe"].get())
    ...
    def refreshLabelSemaineAfficher(self,label):
        ...
        label["text"] = "Semaine actuelle : {0} au {1}".format(startDateString,endDateString)
