...
class CoursForm():
    ...
    def refreshMatiere(*args, entrys, optionMatiere):
        ...
        listOptions = SqliteService.getInstance().selectByQueryEntity(
            "SELECT mat.libelleMatiere FROM MATIERE_ENSEIGNANT AS m_e INNER JOIN ENSEIGNANT AS e ON m_e.k_idEnseignant = e.idEnseignant INNER JOIN MATIERE AS mat ON m_e.k_idMatiere = mat.idMatiere WHERE e.nomEnseignant = '{0}' AND e.prenomEnseignant = '{1}'".format(prenomNomEnseignant[1], prenomNomEnseignant[0]))
    ...
    def form_add_cours(self):
        ...
        sb = Spinbox(container, from_=2019, to=2022,
        ...
        optionClasse = OptionMenuCustom(
            container, listeOptions, entrys, "Classe")
        container.pack(fill=BOTH)
        ...
        listeOptions = ToolsBox.convert_rowlist_tostringlist(listeOptions)
        optionEnseignant = OptionMenuCustom(
            container, listeOptions, entrys, "Enseignant")
        container.pack(fill=BOTH)
        ...
        listeOptions = ToolsBox.convert_rowlist_tostringlist(listeOptions)
        optionMatiere = OptionMenuCustom(
            container, listeOptions, entrys, "Matiere")
        container.pack(fill=BOTH)

        optionEnseignant.v.trace("w", partial(
            CoursForm.refreshMatiere, entrys=entrys, optionMatiere=optionMatiere))

        btn = Button(window, text="Valider", command=partial(
            self.controller.addCours, entrys)).pack(side=BOTTOM)
    ...
    def form_update_cours(self):
        ...
        listeOptions = ToolsBox.convert_rowlist_tostringlist(listeOptions)
        optionClasse = OptionMenuCustom(
            container, listeOptions, entrys, "Jour/Heure/Matiere")
        container.pack(fill=BOTH)
        ...
        btn = Button(window, text="Valider", command=partial(
            self.controller.updateCours, entrys)).pack(side=BOTTOM)
    ...
    def form_delete_cours(self):
        ...
        btn = Button(window, text="Valider", command=partial(
            self.controller.deleteCours, entrys)).pack(side=BOTTOM)
