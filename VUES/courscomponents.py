...
class CoursComponent():
    def __init__(self, canvas, jour, heure, coursRow):
        ...
        self.rectangle = self.canvas.create_rectangle(self.posx - self.width, self.posy - self.height, self.posx,self.posy, fill='#97d1c1')
        self.txtDescription = self.canvas.create_text((self.posx - self.width) + (self.width / 2),(self.posy - self.height) + (self.height / 2), fill="black", text="Cours : {0} \nIntervenant : {1} {2}".format(coursRow["libelleMatiere"], coursRow["nomEnseignant"], coursRow["prenomEnseignant"]))
