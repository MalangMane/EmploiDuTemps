...
menuEleve.add_command(label="Ajout d'un élève", command=eleveForm.form_add_eleve)
menuEleve.add_command(label="Suppresion d'un élève", command=eleveForm.form_delete_eleve)
menuEleve.add_command(label="Association d'un élève à une classe", command=eleveForm.from_associate_eleve)
menuEleve.add_command(label="Afficher les élèves", command=eleveForm.from_display_eleve)
menubar.add_cascade(label = "Eleve", menu=menuEleve)


menuCours = Menu(menubar, tearoff=0)
coursForm = CoursForm(root)
menuCours.add_command(label="Ajout d'un cours", command=coursForm.form_add_cours)
menuCours.add_command(label="Suppression d'un cours", command=coursForm.form_delete_cours)
menuCours.add_command(label="Modification d'un cours", command=coursForm.form_update_cours)
menubar.add_cascade(label = "Cours", menu=menuCours)


menuMatiere = Menu(menubar,tearoff=0)
matiereForm = MatiereForm(root)
menuMatiere.add_command(label="Ajout d'une matière", command=matiereForm.form_add_matiere)
menuMatiere.add_command(label="Suppression d'une matière", command=matiereForm.form_delete_matiere)
...
menuEnseignant.add_command(label="Ajout d'un enseignant" ,command=enseignantForm.form_add_enseignant)
menuEnseignant.add_command(label="Association d'un enseignant", command=enseignantForm.form_associate_matiere)
menuEnseignant.add_command(label="Suppresion d'un enseignant", command=enseignantForm.form_delete_enseignant)
menuEnseignant.add_command(label="Afficher liste des enseignants", command=enseignantForm.form_display_enseignant)
menubar.add_cascade(label= "Enseignant", menu=menuEnseignant)

menuClasse = Menu(menubar, tearoff=0)
classeForm = ClasseForm(root)
menuClasse.add_command(label="Liste d'élèves par classe" ,command=classeForm.form_display_byclasse)
...
btnValider = Button(containerTOP,text="Valider la date", command=partial(edt.selectSemaineByDate,entrys=entrys,label=lbl)).pack(side=LEFT)


listCoursBySemaine = edt.controller.selectCours(edt.currentdate,entrys["Classe"].get())
...
btnPrevious = Button(containerBottom, text="<", width=40,height=2,command=partial(edt.previousWeek,entrys=entrys,label=lbl)).pack(fill=X, side=LEFT)
btnNext = Button(containerBottom, text=">", width=40,height=2,command=partial(edt.nextWeek,entrys=entrys,label=lbl)).pack(fill=X, side=RIGHT)
